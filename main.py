...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Agentic AutoML System"
    )
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        if not execution_output["success"]:
            ...
            refinement = (
                "Simplify model or fix parameters"
            )
        ...
        status = (
            ...
            else evaluation["issue"].capitalize()
        )
        ...
        if evaluation["status"] == "fail":
            ...
            print(
                f"- {clean_text(failure.get('suggestion', 'No suggestion'))}"
            )
        ...
        if prev_metrics:
            ...
            prev_acc = prev_metrics.get(
                "test_accuracy",
                0
            )

            improvement = (
                current_accuracy - prev_acc
            )
    ...
    result_row = {
        ...
        "pipeline_complexity": str(
            best_metrics.get("pipeline_complexity")
        )
    }
